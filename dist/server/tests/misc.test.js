...
describe('## Misc', function () {
  ...
  describe('# Error Handling', function () {
    it('should handle mongoose CastError - Cast to ObjectId failed', function (done) {
      (0, _supertestAsPromised2.default)(_index2.default).get('/api/issues/56z787zzz67fc').expect(_httpStatus2.default.INTERNAL_SERVER_ERROR).then(function (res) {
    ...
    it('should handle express validation error - username is required', function (done) {
      (0, _supertestAsPromised2.default)(_index2.default).post('/api/issues').send({
        mobileNumber: '1234567890'
      }).expect(_httpStatus2.default.BAD_REQUEST).then(function (res) {
        (0, _chai.expect)(res.body.message).to.equal('"code" is required and "context" is required and "message" is required and "selector" is required and "type" is required and "typeCode" is required and "url" is required');
        done();
      }).catch(done);
    });
  });
